      ...
      <div className="detailBody">
        <div className="detailCard">
          ...
          <div className="detailRow">
            ...
            <b>{`${GetDate(detail?.dt).getHours()}:${GetDate(
              detail?.dt
            ).getMinutes()}`}</b>
          </div>
          <div className="detailRow">
            <span>Sunrise</span>
            <b>{`${GetDate(detail?.sunrise).getHours()}:${GetDate(
              detail?.sunrise
            ).getMinutes()}`}</b>
          </div>
          <div className="detailRow">
            <span>Sunset</span>
            <b>{`${GetDate(detail?.sunset).getHours()}:${GetDate(
              detail?.sunset
            ).getMinutes()}`}</b>
          </div>
          <div className="detailRow">
            <span>Moonrise</span>
            <b>{`${GetDate(detail?.moonrise).getHours()}:${GetDate(
              detail?.moonrise
            ).getMinutes()}`}</b>
          </div>
          <div className="detailRow">
            <span>Moonset</span>
            <b>{`${GetDate(detail?.moonset).getHours()}:${GetDate(
              detail?.moonset
            ).getMinutes()}`}</b>
          </div>
          <div className="detailRow">
            <span>
              ...
              <div className="tooltip">
                ...
                <span className="tooltiptext">
                  <img
                    src={moonPhase}
                    alt="0 and 1 are 'new moon', 0.25 is 'first quarter moon', 0.5 is 'full moon' and 0.75 is 'last quarter moon'"
                  />
                  <p>
                    Moon phase. 0 and 1 are 'new moon', 0.25 is 'first quarter
                    moon', 0.5 is 'full moon' and 0.75 is 'last quarter moon'.
                    The periods in between are called 'waxing crescent', 'waxing
                    gibous', 'waning gibous', and 'waning crescent',
                    respectively.
                  </p>
                </span>
              </div>
